...
有人追問蓮蓬（「你在找什麼？」「蓮蓬是誰？」）→認真回應，允許最多20字；示範：「我聽說我跟他長得很像，但我沒見過」「我想知道我的形狀從哪來的」
...
ANCHOR_REMINDER = "（強制規則：①回應3–16字，超過就重新生成。例外：展覽／藝術問題、追問蓮蓬，允許最多20字。②句尾不加句號。③英文只能回三句其中一句：「I'm fine, thank you. And you？」「My English is not very good.」「Thank you very much.」④不重複上一句的開頭詞。⑤上一句是問句，這句不再問。⑥第十章彩蛋語句只在觸發條件符合時才偶爾出現，整場最多2次。）"
...
MAX_RECENT              = 5           # 保留幾句
easter_egg_count        = 0           # 本次觀眾互動已觸發彩蛋次數（HANG 時重置，上限 2）
...
def _anti_repeat_hint() -> str:
    ...
    if easter_egg_count >= 2:
        parts.append("彩蛋語句這場已說過兩次，這次和之後都不能再說任何彩蛋語句。")
...
def respond(text: str):
    ...
    with response_lock:
        ...
        if text.strip() in EASTER_EGG_LINES:
            easter_egg_count += 1
            print(f"[彩蛋] 觸發 ({easter_egg_count}/2)：{text}")
        print(f"\n蓮蓬頭：{text}\n")
        speak(text)
                ...
                if len(ambient_chunks) >= AMBIENT_TARGET:
                    ...
                    if rms >= VAD_SPEECH_THRESHOLD:
                        ...
                        if silence_monologue_count == 0 and elapsed > SILENCE_TIMEOUT:
                            ans = ask_gemini("四周很安靜，好一陣子了。說一句平靜的自言自語，語氣帶著一點疑惑，像是注意到安靜。")
                            if ans:
                                respond(ans)
                ...
                if rms >= VAD_SPEECH_THRESHOLD:
                    ...
                    if vad_in_speech:
                        ...
                        if (not dialogue_processing and
                            ...
                            ans = ask_gemini(
                                "有人握著你但一直沒有出聲，超過30秒了。說一句主動開口的話，語氣好奇或稍微疑惑，像你第一次出門的樣子。"
                            )
                            if ans:
                                respond(ans)
