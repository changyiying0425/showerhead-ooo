...
- 升級 Gemini multimodal 後不再需要 melody 偵測條件，Gemini 直接辨識歌聲／說話／環境音
...
- [x] 記憶系統建立（memory.py + memories.json，初期版本，已改為嵌入 skill.md）
- [x] 全部音訊檔分析完成（21 筆，含 M4A 支援，使用 ffmpeg 轉檔）
- [x] scan_sounds.py：自動掃描新音檔工具（仍保留供未來使用）
- [x] Voicemeeter 音效參數設定（EQ 完成，設定已儲存）
- [x] 麥克風校準（靜音門檻調整為 0.015）
...
- [x] **test_skill.py：11 情境 Gemini 回應品質自動化測試（含字數、英文、角色一致性）**
...
- **ANCHOR_REMINDER**：每次呼叫 Gemini（含 ask_gemini / ask_gemini_audio）前附加字數提醒、英文限制、不重複句型等強制規則
...
- **2026-05-22**：
  ...
  - ask_gemini() 簡化（移除舊 memory 參數），新增 ask_gemini_audio() / numpy_to_wav_bytes()
...
*最後更新：2026-05-22（環境音模式 Gemini multimodal 升級完成、skill.md 完成、待辦項目更新）*
