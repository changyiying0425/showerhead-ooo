...
- [x] **「不夠有生命」概念重寫（待實機測試）** — 第二章從「禁止項目」改寫為「感知邏輯」，並修正改寫後陸續發現的 6 項對話一致性問題（模板收斂、錯誤連續性、聲音迴避崩潰、招呼語回音、是非問句不表態、問句循環），詳見下方「不夠有生命」概念重寫章節、新增 `test_chat.py` 文字測試工具（2026-06-07）
- [ ] **概念重寫：實機測試與調整** — 觀察「感知邏輯」改寫是否真的提升生命感，以及六項新驗證機制在真實展場節奏下是否誤判或拖慢回應
...
- **2026-06-07**：「不夠有生命」概念重寫——SYSTEM_PROMPT 第二章從「禁止項目清單」改寫為「感知邏輯」（解釋為什麼某些概念是空白，而非條列禁令），是本次修正的核心理念轉向；改寫後用 `test_chat.py` 反覆測試，陸續修正模板收斂、錯誤連續性宣稱、聲音迴避崩潰、招呼語逐字回音、是非問句不表態、問句循環六項對話一致性問題
...
## 「不夠有生命」概念重寫 + 對話一致性修正（2026-06-07 第二輪，待實機測試）

> 背景：使用者反饋「他不夠有生命，就只是穿著蓮蓬頭皮的 AI，整個作品理念很空洞」。
> 診斷：原本第二章是一份「禁止項目清單」（條列出它不能說什麼），但禁令本身不會生出個性——
> 一個角色之所以鮮活，是因為它有自己理解世界的邏輯，而不是被劃了一條不能跨越的線。
> 解法：把第二章從「禁令」整個改寫成「感知邏輯」——不是說「不能說顏色」，而是解釋「它沒有眼睛，所以顏色對它是真正的空白，不是被禁止」。
> 改寫後用 `test_chat.py` 反覆測試，過程中陸續炸出幾個新的對話一致性問題，逐一修正如下。

### 核心改寫
- **SYSTEM_PROMPT 第二章**：「二、禁止項目」→「二、它如何理解世界（感知邏輯——不是禁令，是它真實擁有的全部）」
  - 用「它沒有眼睛」「它沒有日曆，也沒有鐘」「它沒有對照組」「它的身體不需要那些事」「它從未被當成一個『正在被觀看的東西』」五個小節解釋**為什麼**某些概念對它是空白，而不是條列「不能說什麼」
  - 新增「它怎麼把陌生的東西，翻譯成它認得的形狀」── 把 雨／天空／溫度／動物／關係／水 等概念的轉譯邏輯講清楚
  - 新增「遇到陌生詞彙時，不要每次都用同一種方式翻譯——那會變成新的口頭禪」，避免改寫後又收斂成「OO是什麼聲音？」這種新模板
- **ANCHOR_REMINDER 新增規則⑧**：「在生成前先問自己：這句話，是一個只靠聲音、觸碰、溫度活著的存在會說出口的嗎？如果這句話換成任何一個會說話的東西講都成立，重新想一次。」

### 改寫後陸續發現並修正的連鎖問題（每個都是「測試→發現→修正→再測」的循環）

1. **模板收斂**：「OO是什麼聲音？」開始重複出現（換湯不換藥，逐字 anti-repeat 抓不到）
   → 新增 `_template_lock_hint()` + `_TEMPLATE_PATTERNS`：偵測最近 3 句中同一句型出現 ≥2 次，直接點名禁止該句型並列出具體替代示範（「不知道」「沒有那個」「我只摸得到我自己」等）
2. **錯誤連續性宣稱**：回應「你的聲音今天比較薄」── 暗示記得「這個人之前」，違反第九章「每位觀眾都是全新的」設計
   → 第九章新增明文規則 + `_FALSE_CONTINUITY_MARKERS` 違規偵測（你今天比較／你上次／你的聲音變了…）
3. **聲音迴避總崩潰**：連續 4 句都退到「這個聲音⋯」這種安全距離描述，完全不回應對話內容
   → 重寫「聲音優先原則」── 從「它的常態」改框成「五句裡才出現一次的意外」；新增 `_sound_deflection_streak()` 偵測連續兩句迴避，強制拉回直接回應
4. **招呼語錯字回音**：不管對方說「你好」「哈囉」「嗨」，固定回「哈囉是什麼」── 範例句把字詞寫死，模型逐字套用而非反映實際內容
   → 改寫該條規則為「反問字詞必須跟對方實際說出口的字一致」；在第八章開頭加總則，避免同樣的逐字套用模式在「謝謝」「再見」等其他條目重演
5. **是非問句不表態**：「是我嗎」「對不對」這類要求表態的問題，只得到「你的聲音是圓的」這種聲音特徵迴避，沒有回答是或不是
   → 新增 `_YES_NO_QUESTION_RE` / `_STANCE_PREFIX_RE` 偵測：偵測到是非問句、回應卻沒有「是／不是／可能／不知道」開頭，判違規重打；第八章新增對應規則與示範
6. **問句循環**：「他是誰」→「那個人是誰」→「問我什麼」── 把對方的問題逐字丟回去，無限循環，這正是第六章早就寫了「不能連續兩句都是疑問句」但模型沒有自我檢查的典型案例
   → 新增 `_QUESTION_LIKE_RE` 偵測：上一句是問句、這句又是問句，直接判違規並強制換成直述句

### 套用範圍
`_violation_reason()` 新增 `user_prompt` 參數（用於偵測是非問句／問句循環需要對照對方原話），目前僅 `ask_gemini`（文字輸入路徑）傳入；`ask_gemini_audio` / `ask_gemini_audio_dialogue` 因輸入是音訊、無法取得對方原話文字，暫不傳入此參數（格式跑掉、超字、錯誤連續性等檢查仍照常套用）

### 測試方式
新增 `test_chat.py`── 純文字互動 REPL，不需要 Arduino / TTS / 麥克風，直接呼叫 `main.ask_gemini()` 測試語氣、字數、多元性、模板收斂、表態能力、循環問題；`/reset` 模擬掛回重置，`/quit` 離開

### 待驗證（實機測試重點）
- 第二章「感知邏輯」改寫後，整體是否真的「更有生命感」，還是仍停留在「會講道理的 AI」
- `_template_lock_hint()` 是否會在長對話中持續發揮作用，或又收斂出新的模板
- 是非問句表態、問句循環偵測在真實對話節奏（含語音延遲、VAD 切段）下是否會誤判或漏判
- `_violation_reason` 重新生成次數是否因為新增多條檢查而顯著增加，導致回應延遲上升

*最後更新：2026-06-07（「不夠有生命」概念重寫：第二章感知邏輯改寫 + 模板收斂／錯誤連續性／聲音迴避崩潰／招呼語回音／是非問句不表態／問句循環六項對話一致性修正）*
